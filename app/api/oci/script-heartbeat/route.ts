...
import { ociScriptHeartbeatBodySchema } from '@/lib/oci/schemas/script-heartbeat-body.zod';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: 'Invalid JSON body', code: 'OCI_SCRIPT_HEARTBEAT_BAD_JSON' },
      { status: 400, headers: getBuildInfoHeaders() }
    );
  }
  const parsed = ociScriptHeartbeatBodySchema.safeParse(bodyUnknown);
  if (!parsed.success) {
    return NextResponse.json(
      {
        ok: false,
        error: 'Invalid body',
        code: 'OCI_SCRIPT_HEARTBEAT_SCHEMA_VIOLATION',
