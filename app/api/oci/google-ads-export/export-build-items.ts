...
export async function buildExportItems(ctx: ExportAuthContext, fetched: FetchedExportData): Promise<BuiltExportData> {
  ...
  const rankedCandidates = [...queueBuild.queueCandidates, ...signalBuild.signalCandidates];
  ...
  const keptConversions = queueBuild.conversions.filter((item) => !rankedIds.has(item.id) || keptIds.has(item.id));
  ...
  return {
    ...
    suppressedQueueIds,
    suppressedSignalIds,
    blockedQueueIds: queueBuild.blockedQueueIds,
