...
export async function POST(req: NextRequest) {
  const retired = assertOutOfCoreSurfaceAllowed('conversations_stage');
  ...
  let body: { conversation_id?: string; stage?: string; next_follow_up_at?: string | null; note?: string | null };
  try {
    body = await req.json();
  } catch {
  ...
  const stage = body.stage != null ? String(body.stage) : '';
  if (!stage.trim()) {
    return NextResponse.json({ error: 'stage is required' }, { status: 400, headers: getBuildInfoHeaders() });
  }
  ...
  const { data, error } = await supabase.rpc('conversation_change_stage_v1', {
  ...
  if (error) {
    return mapConversationRpcError(error, 'Failed to change conversation stage');
  }
