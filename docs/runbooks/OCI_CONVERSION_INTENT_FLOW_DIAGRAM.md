...
>
> **Errata (export):** Sections 3–4 show `offline_conversion_queue` feeding `GET …/google-ads-export` — **obsolete** for the live route, which is **journal-only** (`offline_conversion_queue`). See `docs/architecture/EXPORT_CLOSURE.md`.
...
|---------------------|------------|--------------------|--------------|----------------------------|
| **Operatör görüşüldü** (Görüşüldü) | 60 | V3 Nitelikli Görüşme | `offline_conversion_queue` | `OpsMantik_V3_Nitelikli_Gorusme` |
| **Teklif** | 80 | V4 Sıcak Teklif | `offline_conversion_queue` | `OpsMantik_V4_Sicak_Teklif` |
| **Mühür** (Satış tutarı + kaydet) | 100 | V5 Demir Mühür | `offline_conversion_queue` | `OpsMantik_V5_DEMIR_MUHUR` |

- **Görüşüldü / Teklif:** `POST /api/calls/[id]/seal` body: `{ lead_score: 60 | 80, currency: 'TRY' }`. Seal route call'ı günceller (status=confirmed, confirmed_at, lead_score); sonra **sadece** V3 veya V4 sinyalini `evaluateAndRouteSignal(gear, …)` ile `offline_conversion_queue` tablosuna yazar. **Kuyruğa satır eklenmez.**
...
> **Güncel (export):** `GET /api/oci/google-ads-export` yalnızca **`offline_conversion_queue`** (journal) okur. Aşağıdaki (1)–(2) çift taraflı anlatım **tarihsel**; `offline_conversion_queue`’ı Script batch’ine birleştiren yol kapatılmıştır (`EXPORT_CLOSURE.md`).

1. **Journal:** `offline_conversion_queue` — contacted/offered/won/junk kanon satırları; status: QUEUED → claim → PROCESSING/UPLOADED → ack sonrası COMPLETED (yük yüzeyine göre değişir).
2. **`offline_conversion_queue`:** Audit/hash/recovery ve öncül kontroller için ayrı FSM; **Script GET export bu tabloyu döndürmez.** Eski akışta PENDING sinyaller aynı JSON’da birleştirilirdi — artık değil.
3. **Export:** `GET /api/oci/google-ads-export?siteId=...&markAsExported=true` — Auth: Bearer session_token (verify handshake) veya x-api-key. Dönüş: `{ items, next_cursor, meta… }`; yanıtta uyumluluk için `signals` sayacı **sıfır** kalabilir.
...
- **Mantık hatası riski:**  
  - lead_score 60/80 iken kuyruğa satır eklenmemesi kasıtlı (sadece offline_conversion_queue).  
...
| Export | GET google-ads-export → **journal (`offline_conversion_queue`) only** + hashed_phone where applicable | app/api/oci/google-ads-export/route.ts |
