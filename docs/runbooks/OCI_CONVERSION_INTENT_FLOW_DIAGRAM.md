...
>
> **Errata (export):** Sections 3–4 show `marketing_signals` feeding `GET …/google-ads-export` — **obsolete** for the live route, which is **journal-only** (`offline_conversion_queue`). See `docs/architecture/EXPORT_CLOSURE.md`.
...
> **Güncel (export):** `GET /api/oci/google-ads-export` yalnızca **`offline_conversion_queue`** (journal) okur. Aşağıdaki (1)–(2) çift taraflı anlatım **tarihsel**; `marketing_signals`’ı Script batch’ine birleştiren yol kapatılmıştır (`EXPORT_CLOSURE.md`).
...
2. **`marketing_signals`:** Audit/hash/recovery ve öncül kontroller için ayrı FSM; **Script GET export bu tabloyu döndürmez.** Eski akışta PENDING sinyaller aynı JSON’da birleştirilirdi — artık değil.
