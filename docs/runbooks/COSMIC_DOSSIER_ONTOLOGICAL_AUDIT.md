...
- **[Location]:** `lib/cron/with-cron-lock.ts` (tryAcquireCronLock stores value = Date.now(), never validated by DB); all cron routes that acquire this lock then call Supabase (recover, process-offline-conversions, sweep-unsent-conversions, reconcile-usage, dispatch-conversions, providers/recover-processing).
