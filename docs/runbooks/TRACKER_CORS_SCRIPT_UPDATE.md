...
```html
<script defer src="https://console.opsmantik.com/assets/core.js?v=5" data-ops-site-id="..." data-api="https://console.opsmantik.com/api/sync"></script>
```

Sadece `core.js` → `core.js?v=5` eklenmesi yeterli. Deploy sonrası sayfayı yenileyin (gerekirse hard refresh: Ctrl+Shift+R / Cache temizle). Yeni script ile sync/call-event CORS’a takılmadan gider, intent’ler hem masaüstü hem telefondan düşer.
