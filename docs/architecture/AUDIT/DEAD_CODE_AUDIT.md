...
- `/api/test-oci`, `/api/create-test-site` — used by smoke tests or UI; dev/demo endpoints.
- **`/api/sentry-example-api` removed** (2026-05-12 slimdown) — Sentry sample route deleted with demo pages.
...
- **App pages** — `/test-page` and `/sentry-example-page` **removed** (2026-05-12); use `/dashboard?gclid=...` for local attribution smoke. `/panel` remains for operator CRM deep links.
...
- Historical audit runbooks that referenced `pipeline-service` / `conversion-worker` were pruned in Phase 5 (f5-docs-cleanup). The authoritative module boundaries now live in `docs/architecture/MODULE_BOUNDARIES.md`.
