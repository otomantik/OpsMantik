...
| Google OCI export SSOT | **`offline_conversion_queue` only** for script/API batch upload; see [OCI_VALUE_ENGINES_SSOT.md](./OCI_VALUE_ENGINES_SSOT.md) and [EXPORT_CLOSURE.md](./EXPORT_CLOSURE.md). `offline_conversion_queue` is out of band for that route. Projection is analytics / metrics, not the journal export source. |
...
| Table | Semantics |
|-------|-----------|
| offline_conversion_queue | Append-only; only dispatch_status, google_sent_at updatable. |
