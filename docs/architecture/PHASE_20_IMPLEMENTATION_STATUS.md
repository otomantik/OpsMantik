...
| **2. Edge Iron Dome** | ✅ | `middleware.ts` Düsseldorf geo-fence on `/api/*`; trace headers. (Historical note: `@upstash/ratelimit` was never wired in this tree; app RL uses `lib/services/rate-limit-service.ts` + Upstash Redis.) |
