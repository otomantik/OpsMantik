...
-- Queue visibility hotfix:
-- Junk/cancelled rows themselves must stay out of the pending queue,
-- but they must NOT black-hole other pending click rows from the same session.
...
COMMENT ON FUNCTION public.get_recent_intents_lite_v1(uuid, timestamptz, timestamptz, int, boolean) IS
  'Pending intents only. Junk/cancelled rows stay excluded, but same-session pending siblings remain visible.';
