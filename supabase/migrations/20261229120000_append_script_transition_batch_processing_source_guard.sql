-- PR-D: append_script_transition_batch must only append ledger rows when the journal row
-- is still PROCESSING (script/export claim contract). Prevents silent ledger rows from
-- QUEUED/RETRY/etc. Down: re-apply prior definition from 20261223020200 if ever needed.
...
COMMENT ON FUNCTION public.append_script_transition_batch(uuid[], text, timestamptz, jsonb) IS
  'PR-D: Script-owned batch transition; INSERT filtered to source status PROCESSING only (claim/export contract).';
