-- PR-4D.1: additive row-scoped safe PROCESSING recovery RPC.
-- Keeps legacy recover_stuck_offline_conversion_jobs compatibility path untouched.
...
BEGIN
  ...
  IF v_eligible_count > 0 THEN
    SELECT public.append_sweeper_transition_batch(
      ARRAY(
        SELECT q.id
        FROM public.offline_conversion_queue q
        JOIN unnest(v_ids) input_ids(queue_id) ON input_ids.queue_id = q.id
      ...
      'RETRY',
      now(),
      format('%s|actor=%s', v_reason, left(COALESCE(NULLIF(trim(p_actor), ''), 'processing_recovery_classifier'), 128))
    )
...
COMMENT ON FUNCTION public.recover_safe_processing_queue_rows_v1(uuid[], integer, text, text)
  IS 'PR-4D.1 additive row-scoped PROCESSING recovery path. Recovers only supplied stale PROCESSING rows to RETRY; service_role only.';

REVOKE ALL ON FUNCTION public.recover_safe_processing_queue_rows_v1(uuid[], integer, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.recover_safe_processing_queue_rows_v1(uuid[], integer, text, text) FROM anon;
REVOKE ALL ON FUNCTION public.recover_safe_processing_queue_rows_v1(uuid[], integer, text, text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.recover_safe_processing_queue_rows_v1(uuid[], integer, text, text) TO service_role;
