-- Phase 2: Supabase Nirvana — Authoritative RPC Migration
-- Transition Authority from Application Code to Database RPC.
-- Enforces canonical Pipeline FSM (junk -> contacted -> offered -> won).
...
CREATE OR REPLACE FUNCTION public.apply_call_action_v2(
  p_call_id uuid,
  p_site_id uuid,
  p_stage text, -- canonical: junk, contacted, offered, won
  p_actor_id uuid,
...
BEGIN
  ...
  -- 7. High-Fidelity Ledger Dispatch (Authoritative Outbox Emission)
