-- Command Center KPI RPC (prod was missing from migrations; panel does not call this).
BEGIN;
...
COMMENT ON FUNCTION public.get_command_center_p0_stats_v2(uuid, timestamptz, timestamptz, boolean)
  IS 'Command Center P0 stats: queue/gamification + OCI pipeline counters (caller provides date range).';

ALTER FUNCTION public.get_command_center_p0_stats_v2(uuid, timestamptz, timestamptz, boolean) SECURITY INVOKER;

REVOKE ALL ON FUNCTION public.get_command_center_p0_stats_v2(uuid, timestamptz, timestamptz, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_command_center_p0_stats_v2(uuid, timestamptz, timestamptz, boolean) TO authenticated, service_role;
