-- HunterCard v3 / Predator HUD: expose intel fields from session in get_recent_intents_v2.
-- Adds: utm_term (keyword), matchtype (e/p/b), city, district, device_type for INTEL BOX and TARGET HUD.
    ...
    SELECT COALESCE(
      ARRAY(
        SELECT jsonb_build_object(
          ...
          'ai_tags', s.ai_tags,
          -- HunterCard v3 intel: keyword, match type, location, device
          'utm_term', s.utm_term,
...
IS 'Live Inbox RPC: recent click intents + Hunter AI + intel (utm_term, matchtype, city, district, device_type) for HunterCard v3.';
