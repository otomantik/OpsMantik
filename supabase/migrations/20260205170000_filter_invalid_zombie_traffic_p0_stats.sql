-- Filter Invalid/Zombie Traffic in Command Center stats
--
-- Goal:
-- - Make funnel metrics reflect real humans by excluding "zombie" sessions:
--   sessions.event_count = 0 OR sessions.total_duration_sec < 2
-- - Keep Seals/Revenue counted regardless (edge-case protection)
--
-- Affected RPC:
-- - public.get_command_center_p0_stats_v2
--
-- Notes:
-- - This changes queue_pending + total_leads (+ gclid_leads + avg_scroll_depth) to be based on VALID sessions only.
-- - Sealed + projected_revenue remain based on calls in range (not filtered by session validity).
...
BEGIN
  ...
  -- Sealed + revenue: count regardless of session validity (edge-case protection)
  SELECT
    COUNT(*)::int,
  ...
  RETURN jsonb_build_object(
    ...
    -- Funnel metrics (valid sessions only, except sealed edge-case)
