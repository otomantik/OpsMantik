-- Kuyrukta çöpe gönderilen (junk) intent'lerin tekrar listelenmesini engelle.
-- get_recent_intents_v2 şu an status IN ('intent','confirmed','junk') döndürüyordu;
-- junk ve cancelled artık dönmüyor, böylece çöplenen kartlar "geri gelmez".
-- get_recent_intents_lite_v1 zaten sadece (intent | NULL) döndürüyor; v2 tutarlı olsun.
...
COMMENT ON FUNCTION public.get_recent_intents_v2(uuid, timestamptz, timestamptz, int, boolean) IS
  'Recent intents for queue/dashboard. Excludes junk and cancelled so binned intents do not reappear.';
