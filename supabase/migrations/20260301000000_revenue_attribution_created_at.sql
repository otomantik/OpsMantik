-- Revenue attribution: use calls.created_at (intent date) instead of confirmed_at (seal date)
--
-- Bug: When sealing a yesterday intent, revenue was attributed to today because confirmed_at = NOW().
-- Result: yesterday's kasa becomes 0, today incorrectly gains the revenue.
--
-- Fix: Attribute projected_revenue by created_at (when the lead came in), consistent with sealed count.
-- Sealed count already uses created_at; revenue should match for operator mental model.
