-- Migration: Add isp_asn and is_proxy_detected to sessions (Prompt 1.2 Network & Infrastructure)
-- Date: 2026-02-01
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'sessions' AND COLUMN_NAME = 'isp_asn') THEN
        ALTER TABLE public.sessions ADD COLUMN isp_asn text;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'sessions' AND COLUMN_NAME = 'is_proxy_detected') THEN
        ALTER TABLE public.sessions ADD COLUMN is_proxy_detected boolean DEFAULT false;
    END IF;
END $$;

-- Update get_recent_intents_v2 to return isp_asn and is_proxy_detected (add after telco_carrier in existing RPC)
