-- Migration: Live Inbox v2 - date range RPC for queue
-- Date: 2026-01-29
--
-- Contract:
-- get_recent_intents_v2(
--   p_site_id uuid,
...
-- ) returns jsonb[]
--
-- Source of truth: public.calls (source='click'), filter by created_at in [from,to]
-- Minimal join: sessions only for ads-only filter + attribution/click ids/risk evidence
...
BEGIN
  ...
  IF p_date_from IS NULL OR p_date_to IS NULL THEN
    RAISE EXCEPTION USING
      ...
      DETAIL = 'p_date_from and p_date_to are required',
      ERRCODE = 'P0001';
  END IF;
  ...
  IF v_to < v_from THEN
    RAISE EXCEPTION USING
      MESSAGE = 'invalid_range',
      DETAIL = 'p_date_to must be >= p_date_from',
      ERRCODE = 'P0001';
  END IF;
...
IS 'Live Inbox RPC: recent click intents from calls filtered by absolute date range (UTC). Enriched with risk reasons and OCI pipeline fields.';
