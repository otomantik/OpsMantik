-- Persist canonical V5 occurred_at metadata on calls and emit it into outbox payload.
...
COMMENT ON FUNCTION public.apply_call_action_v1(uuid, text, jsonb, text, uuid, jsonb, integer) IS
  'Seal/confirm branch persists caller phone and canonical sale occurred_at metadata; on seal also writes IntentSealed to outbox_events in the same transaction.';
