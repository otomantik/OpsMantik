-- Migration: Support Optimistic Locking in apply_call_action_v1
-- 1. Update trigger to allow version column updates
...
BEGIN
  ...
  -- note, lead_score, oci_status, oci_status_updated_at, updated_at, and VERSION.
  ...
  THEN
    RAISE EXCEPTION 'calls: only UI fields and version are updatable by app'
      USING ERRCODE = 'P0001';
  END IF;
...
-- 2. Update RPC to handle p_version and auto-increment version
...
BEGIN
  ...
  -- Apply action and INCREMENT version
  ...
  ELSE
    -- Generic update for other actions (link/restore/etc)
    UPDATE public.calls
    SET
