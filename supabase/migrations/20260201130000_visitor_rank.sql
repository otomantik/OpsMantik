-- Migration: Add visitor_rank and previous_visit_count (Prompt 2.2 Returning Giant)
-- Date: 2026-02-01
-- If fingerprint is seen multiple times over 7 days, flag as VETERAN_HUNTER; UI shows High-Frequency Visitor badge.

BEGIN;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'sessions' AND COLUMN_NAME = 'visitor_rank') THEN
        ALTER TABLE public.sessions ADD COLUMN visitor_rank text;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'sessions' AND COLUMN_NAME = 'previous_visit_count') THEN
        ALTER TABLE public.sessions ADD COLUMN previous_visit_count integer DEFAULT 0;
    END IF;
END $$;

-- Update get_recent_intents_v2 to return visitor_rank and previous_visit_count
    ...
    SELECT COALESCE(
      ARRAY(
        SELECT jsonb_build_object(
          ...
          'visitor_rank', s.visitor_rank,
          'previous_visit_count', COALESCE(s.previous_visit_count, 0),
          ...
          'isp_asn', s.isp_asn,
          'is_proxy_detected', COALESCE(s.is_proxy_detected, false),
