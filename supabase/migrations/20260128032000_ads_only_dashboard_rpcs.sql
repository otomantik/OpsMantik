-- Migration: ADS Command Center - Add p_ads_only to dashboard RPCs (default true)
-- Date: 2026-01-28
--
-- Option A: add p_ads_only boolean DEFAULT true
...
-- - get_dashboard_breakdown (kept; filtered)

BEGIN;

-- ============================================
-- get_dashboard_stats (v2.2 contract) + p_ads_only
-- ============================================
...
BEGIN
  ...
  WITH stats AS (
    SELECT
      -- Calls: only include calls that are matched to an Ads-origin session when ads_only=true
      (SELECT COUNT(*)::int
       FROM public.calls c
      ...
      (SELECT COUNT(DISTINCT s.fingerprint)::int
       FROM public.sessions s
       WHERE s.site_id = p_site_id
         AND s.created_month >= v_start_month
         AND s.created_month < v_end_month
...
-- ============================================
-- get_dashboard_timeline + p_ads_only
-- ============================================
...
GRANT EXECUTE ON FUNCTION public.get_dashboard_timeline(uuid, timestamptz, timestamptz, text, boolean) TO anon, authenticated, service_role;

-- ============================================
-- get_dashboard_intents + p_ads_only
-- (incorporates 20260128021000 fix: no first_event_url column usage)
-- ============================================
...
GRANT EXECUTE ON FUNCTION public.get_dashboard_intents(uuid, timestamptz, timestamptz, text, text, boolean) TO anon, authenticated, service_role;

-- ============================================
-- get_dashboard_breakdown + p_ads_only
-- ============================================
...
GRANT EXECUTE ON FUNCTION public.get_dashboard_breakdown(uuid, timestamptz, timestamptz, text, boolean) TO anon, authenticated, service_role;
