-- P4-1 Breakdown v1: get_dashboard_breakdown_v1
-- Returns sources, locations, devices for site + date range.
-- Filters: site_id, created_at BETWEEN p_date_from AND p_date_to, created_month for partition pruning.
-- p_ads_only=true -> AND public.is_ads_session(s)=true (STRICT, no attribution fallback in filter).
-- Output: { "sources": [{ name, count, pct }], "locations": [...], "devices": [...] }
...
BEGIN
  ...
  -- Locations: top 8 + Other. District preferred, else city, else 'Unknown'
  WITH base AS (
    SELECT COALESCE(
...
IS 'P4-1 Breakdown v1: sources (ads_only=true: Google Ads+Other; false: Google Ads/Paid Social/Organic/Direct/Unknown/Other), locations (top 8+Other), devices (Mobile/Desktop/Other/Unknown). Filter: is_ads_session(s) when p_ads_only=true.';
