-- GCLID-first geo: prefer call.district_name (from AdsContext) over session city/district (IP-derived).
-- When calls.location_source = 'gclid' and district_name is set, return that for dashboard; else session geo.
-- Adds location_source to response so UI can show "Source: GCLID" badge.
-- Only patches the Geo / Carrier / Proxy SELECT block; rest unchanged from 20260202021000.
      ...
      ARRAY(
        ...
        FROM (
          SELECT
            ...
            -- Geo: GCLID-first. When call has location_source='gclid' and district_name, use that; else session (IP).
