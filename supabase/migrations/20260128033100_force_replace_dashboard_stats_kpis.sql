-- Migration: ADS Command Center - Force replace get_dashboard_stats KPI payload
-- Date: 2026-01-28
--
-- Some environments can retain older function bodies; this migration drops and recreates
-- the function to guarantee the KPI contract (ads_sessions/high_intent/sealed/cvr).

BEGIN;

DROP FUNCTION IF EXISTS public.get_dashboard_stats(uuid, timestamptz, timestamptz, boolean);
