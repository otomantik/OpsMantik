-- PR-9G.1 follow-up: ensure RETRY transition payload satisfies queue retry invariant.
...
COMMENT ON FUNCTION public.recover_safe_processing_queue_rows_v1(uuid[], integer, text, text)
  IS 'PR-4D.1 additive row-scoped PROCESSING recovery path. PR-9G.1 uses append_worker_transition_batch_v2 and sets RETRY payload fields safely.';
