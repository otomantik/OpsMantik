...
-- Queue fail-closed hotfix:
-- If any row in the same matched_session_id is junk/cancelled,
-- suppress the entire session from the pending queue.
-- Operator intent: junk means "this session is noise", not "show me the sibling row again".
...
COMMENT ON FUNCTION public.get_recent_intents_lite_v1(uuid, timestamptz, timestamptz, int, boolean) IS
  'Pending intents only. Fail-closed queue visibility: sessions with any junk/cancelled row stay hidden.';
