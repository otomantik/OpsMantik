...
-- get_recent_intents_lite_v1(..., p_only_unreviewed, p_include_reviewed)
-- Geo SSOT must prefer sessions.geo_* over legacy sessions.city/district.
...
AS $$
  SELECT
    ...
    s.utm_source,
    s.matchtype,
    coalesce(s.geo_city, s.city) AS city,
    coalesce(s.geo_district, s.district) AS district,
    s.device_type,
    s.device_os,
...
-- Legacy wrapper: keep old 5-arg contract, delegate to canonical 7-arg implementation.
CREATE OR REPLACE FUNCTION public.get_recent_intents_lite_v1(
  p_site_id uuid,
  p_date_from timestamptz,
  p_date_to timestamptz,
...
AS $$
  SELECT
    ...
    x.event_count
  FROM public.get_recent_intents_lite_v1(
    p_site_id,
    p_date_from,
    p_date_to,
    p_limit,
