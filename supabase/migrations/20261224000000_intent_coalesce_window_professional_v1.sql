...
-- Intent coalesce windows (professional tier), aligned with restored contracts:
--   `20260428143000_restore_intent_idempotency_contracts.sql` — single active row per (site_id, intent_stamp)
--   `20260429160000_session_single_card_invariant.sql` — canonical intent_stamp + ledger
--
-- Problem: sub-second / few-second burst gates (`20260503150000`, `20260503160100`) let the same
-- visitor (same fingerprint or same normalized phone) open a second session ~30–120s later, so
-- `ensure_session_intent_v1` legitimately inserts a second head (different session_id → different intent_stamp).
--
-- Policy change (bounded, deterministic):
--   1) Fingerprint session reuse: 4s → 120s (same site + same fingerprint, same calendar month bucket window).
--   2) IP + entry_page session reuse: 900ms → 30s (same NAT + same landing URL, short double-navigation).
--   3) Cross-session phone/WhatsApp burst twin merge on calls: 10s → 180s (same click_intent_burst_match_key_v1).
--
-- Risk note: longer windows can theoretically coalesce different humans on shared devices / corporate NAT.
-- Windows stay short (≤3 min) and require strong signals (fingerprint OR normalized phone key OR IP+URL).

-- ---------------------------------------------------------------------------
-- 1) Widen cross-session burst twin merge (AFTER INSERT trigger on calls)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  WHERE c.site_id = NEW.site_id
    ...
    AND abs(extract(epoch FROM (NEW.created_at - c.created_at))) <= 180
...
-- ---------------------------------------------------------------------------
-- 2) Widen find_or_reuse_session_v1 fingerprint + IP entry burst windows
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_action IN ('phone', 'whatsapp', 'form')
    ...
    WHERE s.site_id = p_site_id
      ...
      AND s.created_month IN (
        ...
        date_trunc('month', v_now - interval '3 minutes')::date
      )
      AND abs(extract(epoch FROM (v_now - s.created_at))) <= 120
  ...
  IF v_action IN ('phone', 'whatsapp', 'form')
    ...
    WHERE s.site_id = p_site_id
      ...
      AND abs(extract(epoch FROM (v_now - s.created_at)) * 1000) <= 30000
    ORDER BY s.created_at DESC
    LIMIT 1;
