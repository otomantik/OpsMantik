...
-- Tighten coalesce windows after `20261224000000_intent_coalesce_window_professional_v1.sql`.
-- Keeps same contract (`intent_stamp` per session) but reduces cross-session coalescence aggressiveness:
--   1) Fingerprint reuse: 120s → 45s
--   2) IP + entry_page reuse: 30s → 5s
--   3) calls burst-twin merge: 180s → 45s
...
BEGIN
  ...
  WHERE c.site_id = NEW.site_id
    ...
    AND abs(extract(epoch FROM (NEW.created_at - c.created_at))) <= 45
...
BEGIN
  ...
  IF v_action IN ('phone', 'whatsapp', 'form')
    ...
    WHERE s.site_id = p_site_id
      ...
      AND abs(extract(epoch FROM (v_now - s.created_at))) <= 45
  ...
  IF v_action IN ('phone', 'whatsapp', 'form')
    ...
    WHERE s.site_id = p_site_id
      ...
      AND abs(extract(epoch FROM (v_now - s.created_at)) * 1000) <= 5000
    ORDER BY s.created_at DESC
    LIMIT 1;
