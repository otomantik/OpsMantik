...
/**
 * Core ingest execution (QStash worker body). Transport stays in `app/api/workers/ingest/*`.
 */
export async function executeIngest(req: NextRequest, lane: IngestLane) {
  ...
  try {
    ...
    if (isRecord(rawBody) && 'kind' in rawBody && rawBody.kind === 'oci_export') {
      ...
      if (!result.ok) {
        const status = result.error === 'UNCLAIMED_FASTPATH' ? 409 : 500;
        return NextResponse.json(
          { success: false, code: result.error ?? 'OCI_EXPORT_FAILED', lane },
          { status }
        );
      }
