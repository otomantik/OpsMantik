...
export async function processOutbox() {
    ...
    try {
        ...
        const timeoutId = setTimeout(() => controller.abort(new DOMException(`Sync timeout after ${SYNC_TIMEOUT_MS / 1000}s`, 'AbortError')), SYNC_TIMEOUT_MS);
        lastFlushAt = now;

        const syncUrl = new URL(CONFIG.apiUrl, typeof window !== 'undefined' ? window.location.origin : undefined);
