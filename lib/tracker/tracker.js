...
function installOutboundIntentHooks() {
    ...
    document.addEventListener('pointerdown', (e) => {
        const el = e.target && e.target.closest
            ? e.target.closest('[data-om-whatsapp], a[href^="whatsapp:"], [class*="jivo"], [id*="jivo"]')
            : null;
        if (!el) return;
        lastPointerContext = {
            ts: Date.now(),
...
export function sendCallEvent(phoneNumber, intentMeta = null) {
    ...
    if (proxyUrl) {
        fetch(proxyUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: payload, keepalive: true }).catch(function (e) {
...
export function initAutoTracking() {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (dataWa && dataWa.getAttribute('data-om-whatsapp')) {
            ...
            const wa = e.target.closest('a[href^="whatsapp:"]');
            if (wa) {
    ...
    document.addEventListener('mouseenter', (e) => {
        const cta = e.target.closest && e.target.closest('a[href^="tel:"], a[href^="whatsapp:"], [data-om-whatsapp], [data-om-cta="true"]');
