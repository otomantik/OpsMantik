...
function extractPhoneIntentFromElement(clickTarget) {
    const el = clickTarget?.closest
        ? clickTarget.closest('a[href^="tel:"], [data-om-phone], [data-phone], [data-tel], [onclick*="tel:"]')
        : null;
    if (!el) return null;
    ...
    const candidate = href?.startsWith('tel:')
        ? href
        : dataPhone
            ? (dataPhone.startsWith('tel:') ? dataPhone : `tel:${dataPhone}`)
            : telFromOnClick;
