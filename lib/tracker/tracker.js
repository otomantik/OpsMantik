 ...
 */
import { CONFIG, getSiteId, resolveIngestBaseOriginForCallEvent } from './config';
...
export function sendCallEvent(phoneNumber, intentMeta = null) {
    if (!siteId) return;
    const session = getOrCreateSession();
    // Derive call-event URL from sync API (data-api /api/sync or edge /opsmantik/sync)
    ...
    if (proxyUrl) {
        fetch(proxyUrl, { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: payload, keepalive: true })
            .then((res) => {
                if (!res.ok) {
        ...
        window.crypto.subtle.importKey('raw', enc.encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign'])
            ...
            .then(async (res) => {
                if (res && !res.ok && typeof console !== 'undefined') {
                    ...
                    try { responseHint = (await res.clone().text()).slice(0, 500); } catch { }
    ...
    const handleIntentClick = (e) => {
        ...
        if (anchor) {
            ...
            if (l.startsWith('tel:') || l.startsWith('callto:') || l.startsWith('sms:')) {
                ...
                // 0ms: stay inside the user-activation window (iOS needs prompt navigation soon after tap). keepalive+fetch still runs.
                setTimeout(() => {
                    try { window.top.location.href = rawHref; }
                    catch { window.location.href = rawHref; }
                }, 0);
                return;
            }
        }
        ...
        if (phoneIntent) {
            ...
            setTimeout(() => {
                try { window.top.location.href = phoneIntent.target; }
                catch { window.location.href = phoneIntent.target; }
            }, 0);
            return;
        }
