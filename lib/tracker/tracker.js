...
function emitTrackedIntent(target, eventAction, label, source, element = null) {
    ...
    try {
        if (typeof sessionStorage !== 'undefined') {
            const storageKey = `om_dedupe_${intentMeta.intentAction}`;
            const stored = sessionStorage.getItem(storageKey);
            if (stored) {
                const age = now - parseInt(stored, 10);
                if (age < 3600000) { // 1 hour
                    if (isTrackerDebugEnabled()) {
                        console.log('[OPSMANTIK][intent] deduped (sessionStorage)', {
                            action: intentMeta.intentAction,
                            target: intentMeta.intentTarget,
                            source
                        });
    ...
    const handleIntentClick = (e) => {
        ...
        if (anchor) {
            ...
            if (anchorHref.toLowerCase().startsWith('tel:')) {
                ...
                setTimeout(() => {
                    try { window.top.location.href = anchorHref; }
                    catch { window.location.href = anchorHref; }
                }, 300);
                return;
            }
        }
