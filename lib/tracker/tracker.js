...
export function sendCallEvent(phoneNumber) {
    if (!siteId) return;
    const session = getOrCreateSession();
    const callEventUrl = window.location.origin + '/api/call-event';
    const scriptTag = document.currentScript || document.querySelector('script[data-ops-site-id]');
...
export function initAutoTracking() {
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') pulse.activeSec += Math.round((Date.now() - pulse.lastActiveAt) / 1000);
        else pulse.lastActiveAt = Date.now();
    });

    setInterval(() => sendEvent('system', 'heartbeat', 'session_active'), CONFIG.heartbeatInterval);
