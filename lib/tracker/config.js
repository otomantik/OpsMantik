...
function deriveSyncProxyUrl(url) {
    ...
    // …/call-event, …/call-event/, …/call-event/v2 (optional ?query #hash) → sibling …/sync
    return url.replace(/\/call-event(?:\/v2)?\/?(?=[?#]|$)/i, '/sync');
}

/**
 * Strip the tracker ingest path (/api/sync or /opsmantik/sync) so signed call-event targets the app origin.
 * @param {string} syncApiUrl absolute URL used by transport (CONFIG.apiUrl)
 * @returns {string} origin (+ optional path prefix) without trailing sync endpoint
 */
export function resolveIngestBaseOriginForCallEvent(syncApiUrl) {
...
function safeResolveUrl(val) {
    ...
    // Relative path support: prepend current origin
    const base = typeof window !== 'undefined' ? window.location.origin : '';
    return base + (val.startsWith('/') ? '' : '/') + val;
}

const resolvedApiUrl = safeResolveUrl(
    syncProxyUrl ||
...
if (typeof window !== 'undefined' && resolvedApiUrl) {
    try {
        ...
        // Same-origin can be intentional when a site routes /opsmantik/* to a worker/proxy.
        const allowedSameOriginProxyPath =
            apiPath.startsWith('/opsmantik/') ||
            apiPath === '/opsmantik/sync' ||
            apiPath === '/metrics/track';
