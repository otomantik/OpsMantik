        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="space-y-1">
            ...
            <div className="text-sm text-muted-foreground">
              {intent.click_id ? (
                <span className="font-mono">{t('common.technical.clickId')}: {intent.click_id.slice(0, 14)}{intent.click_id.length > 14 ? '…' : ''}</span>
              ) : (
                <span className="font-mono">{t('common.technical.clickId')}: —</span>
              )}
            </div>
          </div>
