 ...
 *
 * Kullanım: node scripts/db/_archive/site-specific/oci-muratcan-donusum-analiz.mjs
 *           node scripts/db/_archive/site-specific/oci-muratcan-donusum-analiz.mjs --json              # JSON stdout
 *           node scripts/db/_archive/site-specific/oci-muratcan-donusum-analiz.mjs --json --out=rapor.json  # Dosyaya yaz
 */
...
async function run() {
  ...
  if (report.ozet.failed_retry_toplam > 0) {
    if (report.ozet.hata_kodlari['INVALID_CLICK_ID_FORMAT'] || report.ozet.hata_kodlari['UNPARSEABLE_GCLID']) {
      ...
      console.log('• Hem gclid hem gbraid/wbraid olan satırlar: node scripts/db/_archive/site-specific/oci-muratcan-only-gclid.mjs');
    }
    ...
    console.log('• FAILED/RETRY satırları tekrar kuyruğa almak için: docs/runbooks/_archive/site-specific/oci_muratcan_bugun_tum_durum.sql blok 8 (QUEUED yap) veya worker/cron çalıştır.');
  }
