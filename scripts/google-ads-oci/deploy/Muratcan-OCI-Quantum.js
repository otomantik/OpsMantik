 ...
 * Site: Muratcan AKÜ (muratcanaku.com)
 * V8 Engine Native | Deterministic Sampling | Auto-Healing
 * Kurulum: Google Ads Script Editor'a yapıştır. Script Properties veya aşağıdaki Muratcan değerleri.
 * API key: node scripts/get-muratcan-credentials.mjs
 */

'use strict';

// ========================================================================
// MURATCAN DEFAULTS (Script Properties öncelikli)
// ========================================================================
var MURATCAN_SITE_ID = '28cf0aefaa074f5bb29e818a9d53b488';   // Muratcan AKÜ public_id
var MURATCAN_BASE_URL = 'https://console.opsmantik.com';
var MURATCAN_API_KEY = '3a1a48f946a1f42c584dc15975ff95c2cb2cb0ab23beffc79c5bb03b0fb47726';   // Muratcan sites.oci_api_key

// ========================================================================
// ENV CONFIGURATION (ScriptProperties or Muratcan fallbacks)
// ========================================================================
function getConfig() {
  var props = null;
  try {
  ...
  return Object.freeze({
    SITE_ID: get('OPSMANTIK_SITE_ID', MURATCAN_SITE_ID) || (isLocal ? 'mock-site-id' : ''),
    API_KEY: get('OPSMANTIK_API_KEY', MURATCAN_API_KEY) || (isLocal ? 'mock-api-key' : ''),
...
class UploadEngine {
  constructor() {
    this.columns = [
      ...
      'Conversion value',
      'Conversion currency',
      'Phone'  // Enhanced Conversions: SHA256 hashed E.164 (API'den hashed_phone_number)
    ];
  }
  ...
  process(conversions, opts) {
    ...
    upload.setFileName('Muratcan_OCI_' + new Date().toISOString().replace(/[:.]/g, '-') + '.csv');
    ...
    for (var i = 0; i < conversions.length; i++) {
      ...
      Telemetry.info('Gonderildi: orderId=' + (orderId.length > 50 ? orderId.substring(0, 50) + '...' : orderId) + ' gclid=' + gclidShort + (hashedPhone ? ' +Phone(EC)' : ''), { id: row.id || '' });
...
// ========================================================================
// MAIN EXECUTION — Muratcan AKÜ OCI Sync
// ========================================================================
function main() {
  Telemetry.info('Muratcan AKÜ Quantum OCI Engine Baslatiliyor...');
  var ackUpdated = null;

  try {
    if (!CONFIG.SITE_ID || !CONFIG.API_KEY) {
      Telemetry.wtf('API_KEY gerekli. Script Properties: OPSMANTIK_SITE_ID, OPSMANTIK_API_KEY. Key: node scripts/get-muratcan-credentials.mjs');
      return;
    }
    ...
    if (stats.successIds.length > 0 || (stats.skippedIds && stats.skippedIds.length > 0) || (stats.failedRows && stats.failedRows.length > 0)) {
      Telemetry.info('Muratcan AKÜ OCI senkronizasyonu tamamlandi.');
...
// ========================================================================
// LOCAL TESTING (node Muratcan-OCI-Quantum.js)
// ========================================================================
if (typeof module !== 'undefined' && require !== 'undefined' && require.main === module) {
  console.log('[LOCAL] Muratcan AKÜ OCI Quantum - Local Mode');
    ...
    fetch: function (url, options) {
      ...
      return {
        ...
        getContentText: function () {
          if (url.indexOf('/api/oci/v2/verify') !== -1) return JSON.stringify({ session_token: 'mock_muratcan_token' });
