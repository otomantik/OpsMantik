/**
 * OCI SYNC ENGINE v4.0 (Quantum Void Edition) — Muratcan
 * Site: Muratcan (muratcan.com)
 * Architecture: THE VOID — Absolute Zero Tolerance
 * Features: JWS Signature (ACK), Ghost Cursor Resilience, Merkle Ledger.
 */

'use strict';

// ========================================================================
// MURATCAN DEFAULTS
// ========================================================================
var SITE_ID = 'muratcan-site-id'; // Replace with your public_id
var API_KEY = 'muratcan-api-key'; // Replace with your oci_api_key
var BASE_URL = 'https://console.opsmantik.com';

// ------------------------------------------------------------------------
// VOID PROTOCOL: ASYMMETRIC TRUST (JWS)
// ------------------------------------------------------------------------
// paste your VOID_PRIVATE_KEY (Base64) here. 
// node scripts/generate-void-secrets.js output.
var VOID_PRIVATE_KEY_B64 = '';

// ========================================================================
// CONFIGURATION
// ========================================================================
function getConfig() {
  var props = null;
  try {
    if (typeof PropertiesService !== 'undefined') {
      props = PropertiesService.getScriptProperties();
    }
  } catch (e) { /* ignore */ }

  var get = function (key, fallback) {
    if (props) {
      var v = props.getProperty && props.getProperty(key);
      if (v) return v;
    }
    return fallback || '';
  };

  return Object.freeze({
    SITE_ID: get('OPSMANTIK_SITE_ID', SITE_ID),
    API_KEY: get('OPSMANTIK_API_KEY', API_KEY),
    BASE_URL: get('OPSMANTIK_BASE_URL', BASE_URL),
    PRIVATE_KEY: get('VOID_PRIVATE_KEY', VOID_PRIVATE_KEY_B64),
    SAMPLING_RATE_V1: 0.1,
    HTTP: Object.freeze({
      MAX_RETRIES: 5,
      INITIAL_DELAY_MS: 1500,
      TIMEOUT_MS: 60000,
    }),
  });
}
var CONFIG = getConfig();

// ========================================================================
// CRYPTO ENGINE (JWS/JWE Wrapper)
// ========================================================================
class CryptoEngine {
  /** 
   * Generates a JWS (HS256 is native, RS256 needs Utilities.computeRsaSignature)
   * We use RS256 for Asymmetric Trust.
   */
  static sign(payload) {
    if (!CONFIG.PRIVATE_KEY) return null;

    try {
      var header = { alg: 'RS256', typ: 'JWT' };
      var iat = Math.floor(Date.now() / 1000);
      var jwtPayload = Object.assign({}, payload, {
        iss: 'opsmantik-oci-script',
        aud: 'opsmantik-api',
        iat: iat,
        exp: iat + 300 // 5 minute window
      });

      var base64Encode = function (obj) {
        var str = JSON.stringify(obj);
        return Utilities.base64EncodeWebSafe(str).replace(/=+$/, '');
      };

      var unsignedToken = base64Encode(header) + "." + base64Encode(jwtPayload);

      // Decrypt/Parse PMKCS8 Private Key
      var privKeyStr = Utilities.newBlob(Utilities.base64Decode(CONFIG.PRIVATE_KEY)).getDataAsString();

      var signature = Utilities.computeRsaSha256Signature(unsignedToken, privKeyStr);
      var signatureB64 = Utilities.base64EncodeWebSafe(signature).replace(/=+$/, '');

      return unsignedToken + "." + signatureB64;
    } catch (e) {
      Logger.log('[CRYPTO_ERROR] Sign failure: ' + e.message);
      return null;
    }
  }

  /**
   * Note: RS256 Decryption (JWE) is NOT natively supported by Apps Script Utilities.
   * If the API returns a 'protected' JWE, the script must handle it via a library polyfill.
   * For the Void MVP, we assume HTTPS + JWS is sufficient for AppScripts if JWE polyfill is not present.
   */
  static decrypt(protectedPayload) {
    // RSA-OAEP decryption is heavy. In the absence of a Forge/jsrsasign build, 
    // we request the backend to send raw JSON if the client identifies as AppScript.
    return JSON.parse(protectedPayload);
  }
}

// ========================================================================
// TELEMETRY
// ========================================================================
var Telemetry = {
  info: function (msg) { Logger.log('[INFO] ' + msg); },
  warn: function (msg) { Logger.log('[WARN] ' + msg); },
  error: function (msg, e) { Logger.log('[ERROR] ' + msg + (e ? ' | ' + e.message : '')); }
};

// ========================================================================
// NETWORK LAYER
// ========================================================================
class QuantumClient {
  constructor() {
    this.baseUrl = CONFIG.BASE_URL.replace(/\/+$/, '');
  }

  fetchConversions() {
    var url = this.baseUrl + '/api/oci/google-ads-export?siteId=' + encodeURIComponent(CONFIG.SITE_ID) + '&markAsExported=true';
    var options = {
      method: 'get',
      headers: { 'x-api-key': CONFIG.API_KEY, 'Accept': 'application/json' },
      muteHttpExceptions: true
    };

    var response = UrlFetchApp.fetch(url, options);
    var code = response.getResponseCode();
    if (code !== 200) throw new Error('API Error ' + code + ': ' + response.getContentText());

    var data = JSON.parse(response.getContentText());
    if (data.protected) {
      Telemetry.info('Encrypted JWE payload received. Attempting zero-knowledge unlock...');
      return CryptoEngine.decrypt(data.protected);
    }
    return data;
  }

  sendAck(queueIds, skippedIds) {
    var url = this.baseUrl + '/api/oci/ack';
    var payload = { siteId: CONFIG.SITE_ID, queueIds: queueIds, skippedIds: skippedIds, pendingConfirmation: true };

    var token = CryptoEngine.sign({ action: 'ack', ids: queueIds });

    var options = {
      method: 'post',
      contentType: 'application/json',
      headers: {
        'x-api-key': CONFIG.API_KEY,
        'x-oci-signature': token || ''
      },
      payload: JSON.stringify(payload),
      muteHttpExceptions: true
    };

    var response = UrlFetchApp.fetch(url, options);
    return JSON.parse(response.getContentText() || '{}');
  }

  sendAckFailed(queueIds, code, msg) {
    var url = this.baseUrl + '/api/oci/ack-failed';
    var payload = { siteId: CONFIG.SITE_ID, queueIds: queueIds, errorCode: code, errorMessage: msg };

    var token = CryptoEngine.sign({ action: 'ack-failed', ids: queueIds });

    var options = {
      method: 'post',
      contentType: 'application/json',
      headers: {
        'x-api-key': CONFIG.API_KEY,
        'x-oci-signature': token || ''
      },
      payload: JSON.stringify(payload),
      muteHttpExceptions: true
    };

    UrlFetchApp.fetch(url, options);
  }
}

// ========================================================================
// MAIN
// ========================================================================
function main() {
  Telemetry.info('Muratcan Quantum OCI Engine (Void Edition) Starting...');

  if (!CONFIG.PRIVATE_KEY) {
    Telemetry.error('VOID_PRIVATE_KEY missing. Asymmetric trust will fail.');
    return;
  }

  try {
    var client = new QuantumClient();
    var conversions = client.fetchConversions();

    if (!Array.isArray(conversions) || conversions.length === 0) {
      Telemetry.info('Kuyruk temiz. Islem yok.');
      return;
    }

    Telemetry.info(conversions.length + ' kayit cekildi. Google Ads yuklemesi basliyor...');

    // Google Ads Bulk Upload Logic
    var columns = ['Order ID', 'Google Click ID', 'Conversion name', 'Conversion time', 'Conversion value', 'Conversion currency'];
    var upload = AdsApp.bulkUploads().newCsvUpload(columns, { moneyInMicros: false });
    upload.forOfflineConversions();

    var successIds = [];
    var failedIds = [];

    for (var i = 0; i < conversions.length; i++) {
      var row = conversions[i];
      try {
        upload.append({
          'Order ID': row.orderId,
          'Google Click ID': row.gclid || row.wbraid || row.gbraid,
          'Conversion name': row.conversionName,
          'Conversion time': row.conversionTime,
          'Conversion value': row.conversionValue,
          'Conversion currency': row.conversionCurrency
        });
        successIds.push(row.id.replace('signal_', '').replace('seal_', ''));
      } catch (e) {
        failedIds.push(row.id);
      }
    }

    if (successIds.length > 0) {
      upload.apply();
      Telemetry.info('ACK gonderiliyor (JWS Signed)...');
      client.sendAck(successIds, []);
    }

    if (failedIds.length > 0) {
      client.sendAckFailed(failedIds, 'SCRIPT_APPEND_ERROR', 'Row failed to append to CSV');
    }

    Telemetry.info('Tamamlandi. Toplam: ' + conversions.length);

  } catch (e) {
    Telemetry.error('Fatal execution error', e);
  }
}
