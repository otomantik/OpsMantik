...
test('stage route checks RPC persistence before queue insert', () => {
  const src = readFileSync(STAGE_ROUTE, 'utf8');
  const rpcIndex = src.indexOf("const { data: updatedCall, error: updateError } = await adminClient.rpc('apply_call_action_v1'");
  const errorGuardIndex = src.indexOf('if (updateError) {', rpcIndex);
  const queueInsertIndex = src.indexOf(".from('offline_conversion_queue')");
  ...
  assert.ok(queueInsertIndex > errorGuardIndex, 'queue insert must happen only after RPC success guard');
});
