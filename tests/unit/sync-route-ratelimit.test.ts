...
test('sync rate limit: same IP+UA + different siteId => two buckets, both allowed under load', async () => {
  ...
  const keyA = `site-alpha:${clientId}`;
  const keyB = `site-other:${clientId}`;

  for (let i = 0; i < 80; i++) {
    const r = await RateLimitService.check(keyA, limit, windowMs);
    assert.equal(r.allowed, true, `site-alpha request ${i + 1} should be allowed`);
  }
  for (let i = 0; i < 80; i++) {
  ...
  assert.equal(counts.get('ratelimit:site-alpha:1.2.3.4|Mozilla/5.0'), 80);
