...
test('outbox worker skips lower gears when a higher conversion already exists', () => {
  ...
  assert.ok(src.includes('outbox_signal_skip_higher_gear_exists') || src.includes('SUPPRESSED_BY_HIGHER_GEAR'), 'outbox must log and skip lower gears when a higher one exists');
